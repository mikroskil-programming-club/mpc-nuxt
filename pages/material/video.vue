<script setup>
useSeoMeta({
    title: "Mikroskil Programming Club | Video"
})
...
async function loadData(){
    try{
        ...
        const response = await axios.get('/api/material/video',{headers:{Authorization:`Bearer ${store.token}`}})
...
const handleFilter = async (week) => {
    try{
        ...
        const response = await axios.get(`/api/material/video/${week}`,{headers:{Authorization:`Bearer ${store.token}`}})
...
async function handleDelete(data_id){
    try{
        ...
        showAlert.value = true
        const response = await axios.delete(`/api/material/video`,{data:{_id:data_id},headers:{Authorization:`Bearer ${store.token}`}})
...
<template>
   ...
   <div v-if="datas.length > 0" class="flex justify-center items-center">
           <div class="flex flex-col mt-10 md:hidden divide-y-2 bg-white/30 rounded-xl shadow-md border-2 border-gray-200">
               <div class="flex gap-4 w-[375px] p-4 flex-wrap">
               ...
               <div v-if="selection" class="p-1 items-center flex flex-col gap-4  overflow-none">
                   ...
                   <iframe class="rounded-xl border-2 border-gray-200 shadow-md" v-if="selection" :src="selection.videoLink" width="360px" height="500px" frameborder="0" scrolling="no" allowfullscreen></iframe>
       ...
       <div class="hidden md:flex">
           <div class="flex flex mt-5 w-full h-full p-2 rounded-xl shadow-md border-2 border-gray-200 bg-white/30 backdrop-filter-md justify-center gap-4">
               ...
               <div class="border-gray-200 overflow-none flex flex-col gap-3">
                   ...
                   <iframe class="rounded-lg md:w-[400px] md:h-[450px] lg:w-[600px] lg:h-[550px]" :src="selection.videoLink" frameborder="0" scrolling="no" allowfullscreen></iframe>
               </div>
           </div>
       </div>
