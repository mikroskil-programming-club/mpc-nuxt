<script setup>
useSeoMeta({
    title: "Mikroskil Programming Club | Discussion"
})
...
async function loadData(){
    try{
        ...
        const response = await axios.get('/api/material',{headers:{Authorization:`Bearer ${store.token}`}})
...
const handleFilter = async (week) => {
    try{
        ...
        const response = await axios.get(`/api/material/${week}`,{headers:{Authorization:`Bearer ${store.token}`}})
...
async function handleDelete(data_id){
    try{
        ...
        showAlert.value = true
        const response = await axios.delete(`/api/material`,{data:{_id:data_id},headers:{Authorization:`Bearer ${store.token}`}})
