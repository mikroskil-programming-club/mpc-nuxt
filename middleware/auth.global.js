...
export default defineNuxtRouteMiddleware(async (to, from) => {
    ...
    if(to.path == "/members"){ axios.post('/api/verify',{token:store.token}).catch(()=>{store.reset(); return navigateTo('/login')})}
    if(to.path == "/members" && !store.isAuthenticated && !store.isAdmin) {return navigateTo('/login')}
    if((to.path == "/material" || to.path == "/material/coach" || to.path == "/material/video")){ axios.post('/api/verify',{token:store.token}).catch(()=>{store.reset(); return navigateTo('/login')})}
